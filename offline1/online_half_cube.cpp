...
void drawCylinder(bool half = false,bool invert = false)
{
    glBegin(GL_QUADS);
        if(half == false){
            drawCylinderHelper(
            ...
            GLdouble len1 = length(cylinder[0]);
            Point one = unit(cylinder[0]+ cylinder[1]) * len1;
            Point two = unit(cylinder[2] + cylinder[3]) * len1;
            drawCylinderHelper(
                cylinder[0],one,two,cylinder[3]
            );
        }
        else{
            GLdouble len1 = length(cylinder[0]);
...
void drawCircularSegment(bool half = false)
{
    ...
    glBegin(GL_QUADS);
        if(half == false){
            ...
            GLdouble angle = toRad(curAngle);
            GLdouble len = tlen * sqrt(2.0/3.0) / sin(angle + topAngle);
            Point one = circularSegment[0] + circularSegment[1];
            one = unit(one) * len;
            Point two = circularSegment[2]+ circularSegment[3] ;
            two = unit(two) * len;
            drawCircularSegmentHelper(
                circularSegment[0],
                one,
